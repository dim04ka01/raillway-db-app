...
function WagonTypes() {
    ...
    const fetchItems = useCallback(async () => {
        try {
            const response = await axios.get('/api/wagon-types', {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Удалить тип вагона?')) return;
        try {
            await axios.delete(`/api/wagon-types/${id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            fetchItems();
        } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingItem) {
                await axios.put(`/api/wagon-types/${editingItem.id}`, { name: formName }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                await axios.post('/api/wagon-types', { name: formName }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            setShowModal(false);
            ...
            {showModal && (
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <h2>{editingItem ? 'Редактировать тип вагона' : 'Добавить тип вагона'}</h2>
