...
function Roles() {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm('Удалить роль? Удаление возможно только если нет связанных пользователей.')) return;
        try {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingItem) {
                await axios.put(`/api/roles/${editingItem.id}`, { name: formName }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                await axios.post('/api/roles', { name: formName }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            setShowModal(false);
    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {isAdmin && <button className="btn btn-primary" onClick={openCreateModal}>Добавить роль</button>}
            </div>
            ...
            {showModal && (
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <h2>{editingItem ? 'Редактировать роль' : 'Добавить роль'}</h2>
