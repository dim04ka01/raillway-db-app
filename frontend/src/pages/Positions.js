...
function Positions() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingItem) {
                await axios.put(`/api/positions/${editingItem.id}`, { name: formName }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                await axios.post('/api/positions', { name: formName }, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            setShowModal(false);
    ...
    return (
        <div>
            <div className="flex-between mb-20">
                <h1>Должности</h1>
                {isAdmin && <button className="btn btn-primary" onClick={openCreateModal}>Добавить должность</button>}
            </div>
            ...
            {showModal && (
                <div className="modal-overlay" onClick={() => setShowModal(false)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <h2>{editingItem ? 'Редактировать должность' : 'Добавить должность'}</h2>
